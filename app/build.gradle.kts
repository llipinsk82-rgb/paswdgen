...
val configuredVersionCode = versionCodeProperty?.toIntOrNull()
    ?: if (versionCodeProperty == null) 19 else error("passwdgen.versionCode must be a positive integer")
val configuredVersionName = versionNameProperty?.takeIf { it.isNotBlank() }
    ?: if (versionNameProperty == null) "0.1.9" else error("passwdgen.versionName must not be blank")
