...
val configuredVersionCode = versionCodeProperty?.toIntOrNull()
    ?: if (versionCodeProperty == null) 16 else error("passwdgen.versionCode must be a positive integer")
val configuredVersionName = versionNameProperty?.takeIf { it.isNotBlank() }
    ?: if (versionNameProperty == null) "0.1.6" else error("passwdgen.versionName must not be blank")
